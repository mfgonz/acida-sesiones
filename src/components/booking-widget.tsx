...
export function BookingWidget({ username, slug, durationMinutes }: Props) {
  ...
  if (step === "done") {
    return (
      <div className="rounded-xl border border-ink/10 bg-white p-8 text-center">
        <h2 className="font-display text-lg font-black text-ink">You&apos;re booked!</h2>
        ...
        {confirmation?.meetLink && (
          <a href={confirmation.meetLink} target="_blank" className="mt-4 block text-terracotta hover:underline">
            {confirmation.meetLink}
          </a>
        )}
        {confirmation && (
          <a href={`/cancel/${confirmation.cancelToken}`} className="mt-4 block text-xs text-ink/40 hover:text-ink/70">
            Need to cancel?
          </a>
        )}
        <Link href={`/${username}`} className="mt-6 inline-block text-sm text-terracotta hover:underline">
  ...
  if (step === "form" && selectedSlot) {
    return (
      <div className="rounded-xl border border-ink/10 bg-white p-6">
        <button onClick={() => setStep("pick")} className="mb-4 flex items-center gap-1 text-sm text-ink/60 hover:text-ink">
          <ChevronLeft size={16} /> Back
        </button>
        <p className="mb-1 font-label text-xs uppercase tracking-wide text-ink/50">{durationMinutes} min</p>
        <p className="mb-6 font-display font-bold text-ink">
          {new Intl.DateTimeFormat("en-US", {
            weekday: "long",
            month: "long",
            day: "numeric",
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <button
            type="submit"
            disabled={submitting}
            className="flex w-full items-center justify-center gap-2 rounded-lg bg-ink px-4 py-2.5 text-sm font-medium text-cream transition hover:bg-terracotta disabled:opacity-60"
          >
  ...
  return (
    <div className="grid gap-6 rounded-xl border border-ink/10 bg-white p-6 sm:grid-cols-2">
      <div>
        <div className="mb-3 flex items-center justify-between">
          <p className="font-display font-bold text-ink">{format(month, "MMMM yyyy")}</p>
          <div className="flex gap-1">
            <button
              onClick={() => setMonth((m) => addMonths(m, -1))}
              className="rounded-lg p-1.5 text-ink/50 hover:bg-cream hover:text-ink"
            >
              <ChevronLeft size={16} />
            </button>
            <button
        ...
        <div className="grid grid-cols-7 gap-1 text-center font-label text-xs text-ink/40">
        ...
        {loading && <p className="mt-3 text-xs text-ink/40">Loading availability…</p>}
      </div>
        ...
        <div className="max-h-80 space-y-2 overflow-y-auto pr-1">
          {selectedDaySlots.map((iso) => (
            <button
              ...
              className="block w-full rounded-lg border border-ink/15 py-2 text-sm text-ink hover:border-terracotta hover:bg-terracotta/10"
            >
          ...
          {selectedDate && !selectedDaySlots.length && (
            <p className="text-sm text-ink/40">No times available this day.</p>
          )}
        </div>
        <p className="mt-3 text-xs text-ink/40">Times shown in {timezone}</p>
      </div>
    </div>
  );
}
